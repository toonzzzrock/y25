...
export async function POST(request: Request) {
  try {
    ...
    // Get user from User table to verify password
    console.log("[DEV LOGIN] Querying User table for:", username);
    const [userRows] = await pool.query<RowDataPacket[]>(
      `SELECT u.username, u.password_encrypted, u.salt_random_value FROM \`User\` u WHERE u.username = ?`,
      [username]
    );
    ...
    // Check if user is in developer table (has developer privileges)
    console.log("[DEV LOGIN] Checking developer status for:", username);
    const [devRows] = await pool.query<RowDataPacket[]>(
      `SELECT username FROM \`developer\` WHERE username = ?`,
      [username]
    );

    console.log("[DEV LOGIN] Developer rows found:", devRows.length);

    if (devRows.length === 0) {
      console.log("[DEV LOGIN] User is not a developer:", username);
      return NextResponse.json(
